...
from app.addresses.models import Address, UserAddress
from app.addresses.schemas import SAddressOptional
from app.addresses.utils import add_is_default_to_every_user_address
from app.addresses.countries.dao import CountryDAO
from app.addresses.countries.exceptions import CountryNotFoundException
...
class AddressDAO(BaseDAO):
    ...
    @classmethod
    @manage_session
    async def add(cls, user: Users, session=None, **data):
    ...
    @classmethod
    async def find_all(cls, user: Users):
    ...
    async def _superior_user_find_all(cls, session=None):
        get_all_users_addresses_data_query = (
            ...
            .join(UserAddress, UserAddress.user_id == Users.id)
            .options(
                load_only(Users.email, Users.first_name, Users.last_name),
                joinedload(Users.addresses)
                .joinedload(Address.country)
                .load_only(Country.id, Country.name),
            )
    ...
    @classmethod
    @manage_session
    async def find_by_id(cls, user: Users, address_id: UUID, session=None):
    ...
    @classmethod
    @manage_session
    async def set_to_default(cls, address_id: UUID, user: Users, session=None):
    ...
    async def _change_default_address(
        cls, address_id: UUID, user: Users, session=None
    ):
