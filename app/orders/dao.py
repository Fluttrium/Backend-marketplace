...
from app.addresses.exceptions import AddressNotFoundException
...
from app.orders.lines.router import create_order_line
from app.orders.lines.schemas import SOrderLineCreate
from app.orders.models import Order
from app.orders.statuses.dao import OrderStatusDAO
from app.orders.statuses.exceptions import OrderStatusNotFoundException
from app.payments.methods.dao import UserPaymentMethodDAO
from app.payments.methods.exceptions import ExpiredCardException
...
from app.shipping_methods.exceptions import ShippingMethodNotFoundException
from app.shopping_carts.dao import ShoppingCartDAO
...
from app.shopping_carts.items.models import ShoppingCartItem
from app.shopping_carts.items.router import get_all_shopping_cart_items
...
class OrderDAO(BaseDAO):
    ...
    async def add(cls, user: Users, order_data, session=None):
        ...
        order_total = await cls._calculate_order_total(
        ...
        # Create order
        order = await cls._create(**order_data)

        if not order:
            return None

        await cls._add_every_product_to_order_line(
            user_cart_items, order.id, user
        )
    ...
    async def _add_every_product_to_order_line(
        cls, user_cart_items, order_id, user, session=None
    ):
        for item in user_cart_items.cart_items:
            product_item = await ProductItemDAO.find_one_or_none(
                id=item.product_item_id
            ...
            await create_order_line(order_id, order_line_data, user)
    ...
    @classmethod
    @manage_session
    async def change(cls, order_id, user, data, session=None):
        data = data.model_dump(exclude_unset=True)

        if not await has_permission(user):
            raise_http_exception(ForbiddenException)

        current_order = await cls.find_one_or_none(id=order_id)
        ...
        new_order = get_new_data(current_order, data)

        existing_order = await cls.find_one_or_none(**new_order)

        if existing_order and existing_order.id != order_id:
            raise_http_exception(OrderAlreadyExistsException)

        return await cls.update_data(order_id, data)

    @classmethod
    @manage_session
    async def delete(cls, user, order_id, session=None):
        # Get current order status
        order = await cls.find_one_or_none(id=order_id)
        ...
        await cls._delete_order_lines(order_id)

        # Delete the order
        await cls.delete_certain_item(order_id)

    @classmethod
    @manage_session
    async def _delete_order_lines(cls, order_id, session=None):
    ...
    @classmethod
    @manage_session
    async def find_order_lines(cls, order_id, user, session=None):
        query = (
            select(Order)
